                ...
                <div class="order-details-wrap">
                    <table class="order-details">
                        ...
                        <tbody class="checkout-details">
                            ...
                            <tr>
                                <td>Subtotal</td>
                                <td><strong>{{ rupiah(Cart::getTotal()) }}</strong></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                          ...
                          <div class="card-body">
                            <div class="billing-address-form">
                                    ...
                                    <form method="POST" action="{{ url('proses_checkout') }}">
                                        ...
                                        <div class="form-group">
                                            <label>Alamat</label>
                                            <textarea name="bill" class="form-control" readonly id="bill">{{ getAlamatLengkap(Auth::user()->alamat_user_id) }}</textarea>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <select class="custom-select" required name="bank_id">
                                                ...
                                                @foreach ($bank as $item)
                                                    <option value="{{ $item->id }}">{{ $item->nama_bank.' '.$item->no_rekening }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <select class="custom-select" required name="kurir" id="jasaPengiriman">
                                                ...
                                                <div class="input-group-append">
                                                    <button type="button" class="btn btn-warning" id="cekOngkir">Cek Ongkir</button>
                                                </div>
                                            </div>
                                        </div>
                ...
                success: function(res){
                    ...
                    res.data.forEach(e => {
                        e.costs.forEach(row => {
                            ...
                            $("#listOngkir").append('<div><input class="form-check-input" type="radio" name="jasa_ongkir" id="inlineRadio'+row.service+'" value="'+row.cost[0].value+'"> <label class="form-check-label" for="inlineRadio'+row.service+'">'+row.description+' '+row.cost[0].etd+' hari, Rp.'+row.cost[0].value+'</label></div>');
